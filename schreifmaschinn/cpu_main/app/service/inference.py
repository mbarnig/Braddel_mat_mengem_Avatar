...
def asr_pipe(validated_data,model):
    ...
    # validated_data = validated_data.to(device)
    # print(f">>>>> sent audio data to {device}.")
    
    for chunk in range(chunks):
        print(f"processing chunk {chunk+1}/{chunks}")
        
        relativeStartTime = chunk*30
        

        if chunk==chunks-1:
            validated_data1 = validated_data[relativeStartTime*settings.AUDIO_SAMPLE_RATE:]
            ...
            validated_data1 = validated_data[relativeStartTime*settings.AUDIO_SAMPLE_RATE:(chunk+1)*30*settings.AUDIO_SAMPLE_RATE]
        ...
        try:
            ...
            try:
                validated_data1 = validated_data[relativeStartTime*settings.AUDIO_SAMPLE_RATE:(chunk+1)*30*settings.AUDIO_SAMPLE_RATE-int(0.25*settings.AUDIO_SAMPLE_RATE)]
                result = whisper.transcribe(model, validated_data1, language="lb")
            except Exception as e:
